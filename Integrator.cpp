...
int gaop[MAXLI+3][MAXLI+3][MAXLI+3];
int raop[MAXLI+3][MAXLI+3][MAXLI+3];
double ftab[2*MAXLI+6+1][MAXTABGAM+1];
...
void GammaCoulomb(int m,double t,double rho,double* f)
{
  int i,k,ttab;
...
TABOK:
  ...
  // Set Coulomb as default interaction
  SetInteraction(&GammaCoulomb);
}

// Evaluate overlap integrals
void Integrator::Overlap(double *ra,double *rb,Shell *sa,
 Shell *sb, double **ib,int nsd, bool apply_normalization)
{
  ...
  for (i=0;i<(signed)sa->z.size();i++)
  {
    for (j=0;j<(signed)sb->z.size();j++)
    {
      ...
      rr[2] = sb->z[j]/zp*r[2];
      // s-s type
      v[gaop[0][0][0]] = pow(M_PI/zp,1.5)*exp(-xi*r2);
...
// save and normalize
void Integrator::NormalizeTwoIndex(Shell *sa, Shell *sb, 
...
void Integrator::Kinetic(double *ra,double *rb,Shell *sa,
 Shell *sb, double **ib,int nsd, bool apply_normalization)
{
...
void Integrator::Core(double *ra,double *rb,double *rc,Shell *sa,
 Shell *sb, double **ib,int nsda,int nsdb,bool apply_normalization)
{
  int ax,ay,az,daop,i,j,k,lab,mm,ma;
  double factor,zp,xi,sf,r2,t;
  double r[3],p[3],q[3],f[MAXLI+1];
  double **v,**h;
...
Shell *sa, Shell *sb, Shell *sc,Shell *sd, double blk[MAXNCO][MAXNCO][MAXNCO][MAXNCO],
double eps, bool *skip, int nsda,int nsdb,int nsdc)
{
  ...
  for (i=0;i<(signed)sa->z.size();i++)
  {
    for (j=0;j<(signed)sb->z.size();j++)
    {
      ...
      if (ABS(2.0*M_PI*f1/zp)<eps) goto DOSMIL;
      ...
      for (k=0;k<(signed)sc->z.size();k++)
      {
        for (l=0;l<(signed)sd->z.size();l++)
        {
          ...
          if (ABS(2.0*M_PI*ecd/zq)<eps) goto MIL;
          f2 = f1*ecd*sqrt(xiw/M_PI)*(pow(M_PI*M_PI/(zp*zq),1.5))*
               (pow(sc->z[k],nsdc));
          skip[1] = false;
          Gamma(lt,ew,xiw,f);  // ==================
          for (im=0;im<=lt;im++)
...
/*
void Integrator::Moment(double *ra,double *rb,Shell *sa,
 Shell *sb, double **ib,int sx,int sy,int sz)
{
  int ap,bp,ax,ay,az,i,j,na,nb;

  int l = MAX(sa->l,sb->l) + 1;
  int dover = ((l+1)*(l+2))/2;
  na = ((sa->l+1)*(sa->l+2))/2;
  nb = ((sb->l+1)*(sb->l+2))/2;
  ...
  double ib3[MAXNCO][MAXNCO][MAXNCO];
  ThreeCenterOverlap(ra,rb,rc,sa,sb,sc,ib3);
  mp = raop[sx][sy][sz];
  for (i=0;i<na;i++)
    for (j=0;j<nb;j++)
      ib[ap][j] = ib3[ap][j][bp];       
...
void Integrator::Interaction2(double *ra,double *rc,Shell *sa,Shell *sc,double blk[MAXNCO][MAXNCO], int nsda)
{
  int i,k;
  double oblk[MAXNCO][MAXNCO][MAXNCO][MAXNCO];
  bool skipd[2];
  Shell *sb,*sd;
  ...
  Interaction(ra,ra,rc,rc,sa,sb,sc,sd,oblk,TOLNUM,skipd,nsda,0,0);
  delete sb;
  delete sd;
...
void Integrator::Interaction3(double *ra,double *rb,double *rc,Shell *sa, Shell *sb, Shell *sc, double blk[MAXNCO][MAXNCO][MAXNCO],bool skip, int nsda,int nsdb)
{
  int i,j,k;
  double oblk[MAXNCO][MAXNCO][MAXNCO][MAXNCO];
  bool skipd[2];
  ...
  Interaction(ra,rb,rc,rc,sa,sb,sc,sd,oblk,TOLNUM,skipd,nsda,nsdb,0);
  delete sd;
  skip = skipd[0];
  for (i=0;i<sa->nco;i++)
    for (j=0;j<sb->nco;j++)
...
void Integrator::Dipole(double *ra,double *rb,Shell *sa,
 Shell *sb, double **ib,int indx)
{
